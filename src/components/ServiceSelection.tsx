...
const SERVICE_METADATA_EXT: Record<
  string,
  { steps: string[]; benefits: string[]; advice: string }
> = {
  "sov-cut": {
    steps: [
      "Scalp thermal steaming",
      "Aromatherapy cleansing shampoo",
      "Precision custom shear scissoring",
      "Neck hot shave trim",
      "Finishing style wax",
    ],
    benefits: [
      "Sovereign signature profile alignment",
      "Enhances natural head contours",
      "Scalp hydration and dandruff prevention",
    ],
    advice:
      "Best styled with a light pomade or matte clay. Rebook every 3 to 4 weeks to retain perfect silhouette definition.",
  },
  "buzz-fade": {
    steps: [
      "Consultation for clipper grade",
      "Dual clipper precision crop",
      "Tapered side burn blending",
      "Scalp exfoliating wash",
      "Balancing tonic splash",
    ],
    benefits: [
      "Ultra low-maintenance",
      "Bold head and skull accentuation",
      "Keeps neck feeling light and fresh",
    ],
    advice:
      "Requires minimal styling. Use sunscreen on scalp when outdoors. Ideal for guys seeking high elegance with minimal upkeep.",
  },
  "beard-sculpt": {
    steps: [
      "Hot lavender beard compress",
      "Beard oil nourishing massage",
      "Comb and shear line definition",
      "Clipper length sculpting",
      "Cool hydration beard balm",
    ],
    benefits: [
      "Lengthens or widens jaw aesthetics",
      "Eliminates wild coarse split ends",
      "Relieves dry under-beard itchiness",
    ],
    advice:
      "Comb daily with a boar bristle brush. Apply premium cedar oil at night to sustain exquisite moisture.",
  },
  "royal-shave": {
    steps: [
      "Dual-layer warm pre-shave cream",
      "Hot steam towel wrap",
      "Pure badger-bristle thick lathering",
      "Vintage straight razor close shave",
      "Alum block soothing spray",
    ],
    benefits: [
      "Exfoliates top skin layers",
      "Extremely deep clean shave",
      "Prevents razor bumps and skin redness",
    ],
    advice:
      "Wash your face with cold water for 12 hours after the session. Perfect for premium groomsmen or pre-gala prep.",
  },
  "gent-combo": {
    steps: [
      "Premium haircut + royal straight razor shave",
      "Dual therapeutic thermal compresses",
      "Charcoal facial massage mask",
      "Rejuvenating ear and nose trim",
      "Chilled craft beverage service",
    ],
    benefits: [
      "Complete executive aesthetic upgrade",
      "Unrivaled pure physical decompression",
      "Substantial multi-treatment value pricing",
    ],
    advice:
      "Allow 75 minutes of uninterrupted rest. It is a full physical reset. Ideal prep for weddings, photo shoots, or major career milestones.",
  },
  "royal-combo": {
    steps: [
      "Signature premium haircut + Imperial royal shaving cycle",
      "Scalp massage with natural custom botanicals",
      "Face and hand luxury moisturizer wrap",
      "Exquisite double-edge finish",
    ],
    benefits: [
      "Absolute top tier styling",
      "Max relax experience",
      "Highly discounted bundled valuation",
    ],
    advice:
      "Let our masters guide your style. The top visual configuration possible.",
  },
};
...
export function ServiceSelection({
  ...
  const matchedMetadata = activeSheetService
    ? SERVICE_METADATA_EXT[activeSheetService.id]
    : null;
    ...
    <div className="space-y-6">
      <div className="text-center sm:text-left space-y-2">
        <h2 className="text-2xl font-light tracking-wide text-stone-100 uppercase">
          <Translate id="services_title_main" fallback="Select Haircut &" />{" "}
          <span className="text-amber-400 font-semibold">
            <Translate
              id="services_title_accent"
              fallback="Treatment Packages"
            />
          </span>
        </h2>
        <p className="text-sm text-stone-400 font-light">
          <Translate
            ...
            fallback="Choose from our carefully curated menu of luxury grooming treatments."
          />
        </p>
      </div>

      {/* Category Tabs: Swipeable on Mobile, Wrapped on Desktop */}
      <div className="overflow-x-auto overflow-y-hidden scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0">
            ...
            >
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="py-2.5 space-y-1.5 max-w-full">
                          {SERVICE_METADATA_EXT[service.id]?.steps.map(
                            (step, sIdx) => (
                              <div
                                key={sIdx}
                                className="flex items-center gap-2 text-[11px] text-stone-400 font-light"
                              >
              ...
              <div className="flex items-center justify-between mt-4">
                <span className="text-[10px] font-mono uppercase tracking-widest text-stone-500">
                  {isSelected
                    ? t("chair_reserved", "Chair Reserved")
                    : t("select_experience", "Select Experience")}
                </span>
                <span
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs font-bold font-mono text-amber-450">
                        ${service.price}
                      </span>
                    ...
                    <div className="pt-2.5 mt-2 border-t border-stone-850/60 text-left space-y-2">
                      ...
                      <div className="bg-stone-950/40 rounded-lg p-2 border border-stone-850/40 font-sans">
                        ...
                        <div className="grid grid-cols-1 gap-1">
                          {SERVICE_METADATA_EXT[service.id]?.steps
                            .slice(0, 3)
                            .map((step, sIdx) => (
                              <div
                                key={sIdx}
      ...
      <BottomSheet
        ...
        title={
          activeSheetService
            ? `${t("service_" + activeSheetService.id + "_name", activeSheetService.name)} ${t("ritual", "Ritual")}`
            : ""
        }
      >
        {activeSheetService && matchedMetadata && (
          <div className="space-y-6">
            <div className="flex items-center justify-between bg-stone-900/60 border border-stone-800 p-4 rounded-xl">
              <div>
                <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-widest">
                  {t("pricing_rate", "Pricing Rate")}
                </span>
                <p className="text-2xl font-black text-amber-400 font-mono">
                  ${activeSheetService.price}
                </p>
              </div>
              <div className="text-right">
                <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-widest">
                ...
                <p className="text-md font-bold text-stone-200 font-mono">
                  {activeSheetService.duration} {t("minutes_suffix", "minutes")}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-2 bg-stone-900/30 rounded-xl p-4 border border-stone-850">
                {matchedMetadata.steps.map((step, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-2 text-xs text-stone-400 font-light"
                  >
                    <span className="font-mono text-amber-550 font-bold min-w-4 font-mono">
                      0{idx + 1}.
                    </span>
                    <span>{step}</span>
                  </li>
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-mono font-bold uppercase tracking-widest text-stone-300">
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-mono font-bold uppercase tracking-widest text-stone-300">
                {t("styling_advice", "Styling Advice")}
              </h4>
            ...
            <div className="pt-4 border-t border-stone-850">
              ...
              >
                ...
                {t("select_this_experience", "Select This Experience")}
              </button>
            </div>
          </div>
        )}
