...
const BARBER_METADATA_EXT: Record<
  string,
  {
    bio: string;
    motto: string;
...
> = {
  "any-barber": {
    ...
    motto:
      "Maximum efficiency without compromising a single strand of Royal Grooming standards.",
    signatureService: "All classic haircuts & rapid premium cleanups",
    tools: [
      "Gold barber taper clippers",
      "Dual straight razors",
      "Fine titanium scissor shear pairings",
    ],
    achievements: [
      "Sovereign Punctuality Laurel",
      "Top Client Comfort Gold Star",
    ],
  },
  "alex-vance": {
    ...
    motto:
      "Precision is not mechanical. It is the architectural alignment of individual character.",
    signatureService: "Imperial Scissor Cuts & High-Fashion Contours",
    tools: [
      "Serrated titanium shearing tools",
      "Sterling vintage cutthroats",
      "Natural oil balms",
    ],
    achievements: [
      "Metropolitan Stylist Champion 2024",
      "Master Craftsman License Certification",
    ],
  },
  "marcus-sterling": {
    ...
    motto:
      "Every jawline has the capacity for authority. My blades merely bring it out.",
    signatureService: "Sovereign Beard Sculpting & Razor Sharp Skin Fades",
    tools: [
      "High-torque custom detail clippers",
      "Authentic wooden cutthroat razor",
      "Rosewood beard combs",
    ],
    achievements: [
      "National Barbering Cup Winner 2023",
      "Grooming Council Lifetime Elite Member",
    ],
  },
  "elena-rostova": {
    ...
    motto:
      "The royal shave is not a chore. It is a deeply restorative luxury meditation.",
    signatureService:
      "Imperial Shaves, Hot-Towel Massages & Combination Rituals",
    tools: [
      "Badger hair leather foam brushes",
      "Vapo thermal facial steamers",
      "Cold steel facial rollers",
    ],
    achievements: [
      "Elite Thermal Shave Innovator Award 2024",
      "Spa Therapy Master Guild Member",
    ],
  },
  "julian-croft": {
    ...
    motto:
      "Style never stops moving. Stay adaptive, crisp, and preserve your texture.",
    signatureService: "Textured Crop Fades, High Tapers & Street Styling",
    tools: [
      "Cordless magnetic motor clippers",
      "Texture razors",
      "Matte finishing clay formulas",
    ],
    achievements: [
      "Top 30 Under 30 Global Groomers Winner",
      "Urban Street-Styling Laureate",
    ],
  },
};
...
export function BarberSelection({
  ...
  const { t } = useLanguage();
  const [activeSheetBarber, setActiveSheetBarber] = useState<Barber | null>(
    null,
  );
  ...
  const renderAvatar = (barberId: string, avatarClass: string) => {
    switch (barberId) {
      case "any-barber":
        return (
          <div
            className={`relative w-20 h-20 rounded-full flex items-center justify-center border-2 bg-stone-900 border-amber-500/30`}
          >
      ...
      case "alex-vance":
        return (
          <div
            className={`relative w-20 h-20 rounded-full flex items-center justify-center border bg-stone-900 border-indigo-500/20`}
          >
            <span className="text-xl font-serif font-bold text-indigo-350">
              AV
            </span>
      ...
      case "marcus-sterling":
        return (
          <div
            className={`relative w-20 h-20 rounded-full flex items-center justify-center border bg-stone-900 border-emerald-500/20`}
          >
            <span className="text-xl font-serif font-bold text-emerald-350">
              MS
            </span>
      ...
      case "elena-rostova":
        return (
          <div
            className={`relative w-20 h-20 rounded-full flex items-center justify-center border bg-stone-900 border-rose-500/20`}
          >
            <span className="text-xl font-serif font-bold text-rose-350">
              ER
            </span>
      ...
      case "julian-croft":
        return (
          <div
            className={`relative w-20 h-20 rounded-full flex items-center justify-center border bg-stone-900 border-sky-500/20`}
          >
            <span className="text-xl font-serif font-bold text-sky-350">
              JC
            </span>
  ...
  const currentSheetData = activeSheetBarber
    ? BARBER_METADATA_EXT[activeSheetBarber.id]
    : null;
    ...
    <div className="space-y-6">
      <div className="text-center md:text-left space-y-2">
        <h2 className="text-2xl font-light tracking-wide text-stone-100 uppercase">
          <Translate id="barbers_title_main" fallback="Choose Your" />{" "}
          <span className="text-amber-400 font-semibold">
            <Translate id="barbers_title_accent" fallback="Stylist" />
          </span>
        </h2>
        <p className="text-sm text-stone-400 font-light">
          <Translate
            id="barbers_subtitle"
            fallback="Our team is composed of certified master barbers. Choose your preferred expert."
          />
        </p>
      </div>
        ...
        {visibleBarbers.map((barber) => {
          ...
          return (
            <motion.div
              ...
              className={`relative flex flex-col sm:flex-row items-center gap-5 p-5 rounded-2xl select-none transition-all duration-300 border-2 ${
                isOnBreak
            ...
            >
              ...
              {/* Text info */}
              <div className="flex-grow text-center sm:text-left space-y-1.5 w-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-center sm:justify-between gap-1">
                  ...
                  <button
                    ...
                    className="w-9 h-9 mx-auto sm:mx-0 rounded-xl bg-stone-900/90 border border-stone-800 flex items-center justify-center text-stone-400 hover:text-amber-400 active:scale-90 transition-transform cursor-pointer"
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="flex sm:hidden overflow-x-auto overflow-y-hidden gap-4 pb-4 scrollbar-none snap-x snap-mandatory -mx-4 px-4"
      >
        {visibleBarbers.map((barber) => {
          const isSelected = selectedBarber?.id === barber.id;
          const isAnyBarber = barber.id === "any-barber";
      ...
      <BottomSheet
        ...
        title={
          activeSheetBarber
            ? `${t("stylist", "Stylist")}: ${t("barber_" + activeSheetBarber.id + "_name", activeSheetBarber.name)}`
            : ""
        }
      >
          ...
          <div className="space-y-6 text-stone-300 font-sans">
            <div className="flex items-center gap-4 bg-stone-900/60 p-4 rounded-xl border border-stone-800">
              <div className="w-16 h-16 rounded-full border border-stone-700 bg-stone-950 flex items-center justify-center text-amber-400 text-xl font-bold font-serif">
                {activeSheetBarber.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")}
              </div>
              <div className="space-y-1">
                <h4 className="font-bold text-stone-250 text-md">
                  {t(
                    "barber_" + activeSheetBarber.id + "_name",
                    activeSheetBarber.name,
                  )}
                </h4>
                <div className="flex items-center gap-1.5">
                  <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-500" />
                  <span className="text-xs font-mono font-bold text-amber-400">
                    {activeSheetBarber.rating.toFixed(1)}{" "}
                    {t("rating_label", "Rating")}
                  </span>
                  <span className="text-xs text-stone-500">
                    ({activeSheetBarber.reviewsCount}{" "}
                    {t("reviews_count", "verified reviews")})
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h5 className="text-xs font-mono font-bold uppercase tracking-widest text-stone-450">
                {t("creative_motto", "Creative Motto")}
              </h5>
            ...
            <div className="space-y-2">
              <h5 className="text-xs font-mono font-bold uppercase tracking-widest text-stone-450">
                {t("biography_background", "Biography & Background")}
              </h5>
            ...
            <div className="space-y-3">
              <h5 className="text-xs font-mono font-bold uppercase tracking-widest text-stone-450">
                {t("crafting_tools", "Crafting Tools of Selection")}
              </h5>
              <div className="flex flex-wrap gap-1.5">
                {currentSheetData.tools.map((tool, tIdx) => (
                  <span
                    key={tIdx}
                    className="px-3 py-1 bg-stone-900 border border-stone-850 rounded-full text-[10px] font-mono text-stone-400"
                  >
                    {tool}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {currentSheetData.achievements.map((ach, aIdx) => (
                  <div
                    key={aIdx}
                    className="flex items-center gap-2 text-xs text-stone-450"
                  >
